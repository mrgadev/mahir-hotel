        ...
        <main class="h-full overflow-y-auto">
            <div class="container mx-auto">
                <div class="flex w-full gap-5 mx-auto justify-between items-center">
                    ...
                    <a href="{{route('dashboard.promo.create')}}" class="flex items-center gap-2 mt-10 px-5 py-2 border-2 rounded-md bg-primary-100 p-2 text-primary-700 hover:bg-white transition-all duration-75 hover:text-[#976033] text-base text-center">
                                ...
                                @forelse ($promos as $promo)
                                    <tr class="cursor-pointer">
                                        <td scope="row" class="px-4 pe-0 py-4 font-medium text-gray-900 whitespace-nowrap">
                                            <input type="checkbox" name="promo_ids[]" class="cursor-pointer child-checkbox w-5 h-5 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600" value="{{ $promo->id }}">
                                        </td>
                                        <td class="font-medium text-gray-900 whitespace-nowrap">{{$promo->id}}</td>
                                        <td class="">
                                            <img src="{{Storage::url($promo->cover)}}" alt="" class=" w-10 object-cover object-top transition duration-500 mb-2">
                                        </td>
                                        ...
                                        <td class="flex items-center">
                                            ...
                                            <form action="{{route('dashboard.promo.destroy', $promo)}}" class="" method="POST">
                                                @method('DELETE')
                                                @csrf
...
@push('addon-script')
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const actionUrl = "{{ route('dashboard.promo.bulkDelete') }}";

            deleteButton.addEventListener('click', function () {
                const promoIds = Array.from(document.querySelectorAll('input[name="promo_ids[]"]:checked'))
